...
def make_movie_with_potentiostat_data(txm_scan_numbers,biologic_file, image_used_for_plot, movie_time_span_seconds, seconds_per_movie_frame, output_filename):
    ...
    animation_handle=animation.FuncAnimation(fig_han, change_imshow, frames=range(int(movie_time_span_seconds/seconds_per_movie_frame)), blit=True, interval=100, repeat=False)
    #writer = animation.FFMpegFileWriter(fps=15, bitrate=10000, codec="libx264")
    animation_handle.save('test.gif', writer='imagemagick',fps=15, bitrate=10000)
    plt.close()
